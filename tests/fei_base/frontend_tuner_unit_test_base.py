...
class FrontendTunerTests(ParameterizedTestCase,unittest.TestCase):
    ...
    def testFRONTEND_3_2_18(self):
        ...
        if high==0:
            self.check(True, False,'Upper bandwidth range set to 0, cannot test above highest bandwidth', successMsg='info',throwOnFailure=True)   
    ...
    def testFRONTEND_3_3_19(self):
        ...
        try:
            ...
            try:
                ...
                self.checkAlmostEqual(getMinGain(idx=0,ttype=ttype),tuner_control.getTunerGain(controller_id),'In-bounds setting of gain - set to minimum gain (%s)'%getMinGain(idx=0,ttype=ttype),places=2)
                tuner_control.setTunerGain(controller_id, getMaxGain(idx=0,ttype=ttype))
                self.checkAlmostEqual(getMaxGain(idx=0,ttype=ttype),tuner_control.getTunerGain(controller_id),'In-bounds setting of gain - set to maximum gain (%s)'%getMaxGain(idx=0,ttype=ttype),places=2)
    ...
    def _testBULKIO(self,tuner_control,comp_port_name,comp_port_type,ttype,controller,listener1=None,listener2=None):
        ...
        streamSink2 = sb.StreamSink()#comp_port_type)
        streamSink3 = sb.StreamSink()#comp_port_type)
        streamSink4 = sb.StreamSink()#comp_port_type)
        streamSink5 = sb.StreamSink()#comp_port_type)
    ...
    def testFRONTEND_RF_Info_Pkt(self):
        ...
        controller['CF']= getValueInRange(DEVICE_INFO['capabilities'][0]['RX_DIGITIZER']['CF'])
    ...
    def testFRONTENDCheckRFwithRFInfo(self):
        ...
        rf_info_pkt = self._generateRFInfoPkt(rf_freq=10000e6,if_freq=100e6,rf_bw=10e6)
        rfInfo_port._set_rfinfo_pkt(rf_info_pkt)
        controller = generateTunerRequest()
        controller['CF'] = 10200e6
    ...
    def check(self, A, B, message, throwOnFailure=False, silentFailure=False, silentSuccess=False, indent_width=0, failureMsg='FAILURE', successMsg='ok',settestname=True):
        ...
        if settestname:
            ...
            # Data Flow tests have more levels before you get to the high level test name
            if self.currentTestName in("_testBULKIO","_testSDDS","_testvita49"):
                self.currentTestName = inspect.stack()[3][3]
            elif self.currentTestName in("_compareAttach","_verifyAttach","_verifyVITA49Attach","_verifySRI"):
                self.currentTestName = inspect.stack()[4][3]
    ...
    def checkAlmostEqual(self, A, B, message, throwOnFailure=False, silentFailure=False, silentSuccess=False, indent_width=0, failureMsg='FAILURE', successMsg='ok', places=7,settestname=True):
        # successMsg suggestions: PASS, YES, ok, u'\u2714' (check mark)
        # failureMsg suggestions: FAIL, NO, u'\u2718' (x mark)
        #print "DEBUG","A=",A,"B=",B
        if settestname:
